...
<script>
    ...
    export default {
        ...
        mounted(){
            ...
            this.mySpRoles = myAllRoles.filter(r=>r.level==='sp');
            this.totalColumn = this.mySpRoles.length;
        },
        computed:{
            filterRoles(){
                let{mySearchName,mySpRoles,mySelectedType,currentPage} = this;
                let arr=[...mySpRoles];
                if(mySearchName.trim()){
                    arr =mySpRoles.filter(r=>r.name.indexOf(mySearchName)!==-1);
                }
                if(mySelectedType.trim()){
                    arr = mySpRoles.filter(r=>r.type.indexOf(mySelectedType)!==-1)
                }
                let showList = [];
